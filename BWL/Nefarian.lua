...
BigWigsNefarian.revision = tonumber(string.sub("$Revision: 18327 $", 12, -3))
		...
		if string.find(msg, i) then
			if v[2] then
				...
				if self.db.profile.otherwarn and string.find(msg, L["landing_soon_trigger"]) then 
			           ...
			           self:TriggerEvent("BigWigs_StartBar", self, L["land"], 165, "Interface\\Icons\\INV_Misc_Head_Dragon_Black")
			           self:TriggerEvent("BigWigs_StartBar", self, L["Mob_Spawn"], 10, "Interface\\Icons\\Spell_Holy_PrayerOfHealing")
		                   self:ScheduleEvent("BigWigs_Message", 135, L["landing_soon_warning"], "Important", true, "Alarm")
		                   self:ScheduleEvent("BigWigs_Message", 155, L["landing_very_soon"], "Important", true, "Long")
